...
func runProductionContractLoad(t *testing.T, prodNet *ProductionNetwork, totalTrades int, numBlocks int) {
	...
	t.Logf("\n─────────────────────────────────────────────────────────")
	t.Log("Connecting to validator RPC...")
	t.Logf("─────────────────────────────────────────────────────────")

	// Use first validator for RPC connection
	rpcAddr := prodNet.GetValidatorRPCAddr(0)
	if rpcAddr == "" {
		t.Fatal("Failed to get validator RPC address")
		return
	}
	t.Logf("RPC Address: %s", rpcAddr)

	// Get current block height before starting
	initialHeight, err := getBlockHeight(t, rpcAddr)
	if err != nil {
		t.Fatalf("Failed to get initial block height: %v", err)
		return
	}
	...
	for blockNum := 0; blockNum < numBlocks; blockNum++ {
		...
		// 1. Deploy the BatchOrderBook contract (if not already deployed)
		// 2. Generate trade data
		// 3. Sign and submit transactions to executeBatchTrades
		// 4. Handle transaction receipts and errors
		t.Logf("Submitting %d contract transactions...", tradesPerBlock)
		currentHeight := initialHeight + int64(blockNum)

		// Wait for block to be mined instead of simulating
		targetHeight := currentHeight + 1
		t.Logf("Waiting for block %d to be mined (current: %d)...", targetHeight, currentHeight)

		err := waitForBlock(t, rpcAddr, targetHeight, 30*time.Second)
		if err != nil {
			t.Logf("Warning: failed to wait for block %d: %v", targetHeight, err)
			// Continue anyway for demonstration purposes
			time.Sleep(2 * time.Second)
		} else {
			t.Logf("✓ Block %d mined successfully", targetHeight)
		}
...
// getBlockHeight queries the current block height from the RPC endpoint
func getBlockHeight(t *testing.T, rpcAddr string) (int64, error) {
	// Use CometBFT RPC to get block height
	// The endpoint is /status which returns node status including latest block height
	url := fmt.Sprintf("%s/status", rpcAddr)

	resp, err := http.Get(url)
	if err != nil {
		return 0, fmt.Errorf("failed to query RPC status: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf("RPC status returned non-OK status: %d", resp.StatusCode)
	}
	...
	// Convert string height to int64
...
func waitForBlock(t *testing.T, rpcAddr string, targetHeight int64, timeout time.Duration) error {
	...
	for {
		...
		case <-ticker.C:
			currentHeight, err := getBlockHeight(t, rpcAddr)
			if err != nil {
				t.Logf("  Warning: failed to get block height: %v", err)
				continue
			}
			...
			if time.Now().Unix()%2 == 0 {
				t.Logf("  Current height: %d, waiting for: %d", currentHeight, targetHeight)
			}
		}
	}
}
